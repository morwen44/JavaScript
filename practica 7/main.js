...
const productCost = (productArray) => {
  productArray.forEach((producto) => {
    let { name, priceMXN } = producto;
    console.log(`El producto ${name} tiene un costo de ${priceMXN} MXN`);
  });
};

productCost(productos);
...
const exchangePrices = (productArray, exchange) => {
  let { dollar, euro, pound } = exchange;

  let productsInt = productArray.map((product) => ({
    ...product,
    priceInDollar: (product.priceMXN * dollar).toFixed(2),
    priceInEuro: (product.priceMXN * euro).toFixed(2),
    priceInPound: (product.priceMXN * pound).toFixed(2),
  }));

  console.log(productsInt);
};
exchangePrices(productos, exchangeRates);

/*
      3.- Obtener el costo promedio de los objetos de la lista (en mxn)
  */

const getAverage = (productArray) => {
  let prices = productArray.map((item) => item.priceMXN);
  let suma = prices.reduce((sum, number) => sum + number, 0);
  let average = suma / productArray.length;
  return average;
};

let averagePrice = getAverage(productos);
console.log(`Average price ${averagePrice.toFixed(2)}`);
/*
      4.- Obtener una nueva lista con aquellos productos cuyo precio sea mayor al promedio (en mxn)
  */

const priceFilter = (productArray, precio) =>
  productArray.filter((item) => item.priceMXN > precio);

console.log(priceFilter(productos, averagePrice));
/*
      5.- Obtener una lista de las categorías de los productos sin repetir
  */

const allcategories = (productArray) => {
  let productCat = productArray.map((item) => item.category).flat();
  let uniqueCategories = [...new Set(productCat)];
  return uniqueCategories;
};

let categories = allcategories(productos);
